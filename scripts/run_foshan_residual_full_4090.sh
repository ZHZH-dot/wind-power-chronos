...
OUTPUT_ROOT="${OUTPUT_DIR:-results/fine_tune/foshan_chronos2_residual/full_${RUN_ID}}"
...
python -m src.training.foshan_residual_finetune \
  ...
  --fine-tune-mode full --stage dry-run --output-dir "${OUTPUT_ROOT}/dry_run"
python -m src.training.foshan_residual_finetune \
  --input "${INPUT}" --config "${CONFIG}" --model-path "${SNAPSHOT}" \
  --fine-tune-mode full --stage smoke --output-dir "${OUTPUT_ROOT}/smoke" \
  --dataloader-num-workers 0
python -m src.training.foshan_residual_finetune \
  --input "${INPUT}" --config "${CONFIG}" --model-path "${SNAPSHOT}" \
  --fine-tune-mode full --stage search --output-dir "${OUTPUT_ROOT}/search" \
  --dataloader-num-workers 0

echo "Full-tuning candidate manifests: ${OUTPUT_ROOT}/search/*/training_manifest.json"
